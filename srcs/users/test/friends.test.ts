...
describe("Friends API", () => {
  ...
  describe("POST /users/friends", () => {
    test("Should add friend successfully - 201", async () => {
      vi.spyOn(friendsService, "addFriend").mockResolvedValue(
  ...
  describe("DELETE /users/friends/:targetId", () => {
    test("Should delete own friendship - 200", async () => {
      vi.spyOn(friendsService, "removeFriend").mockResolvedValue(
        mockFriendship as any,
      );
    ...
    test("Should return 403 if not admin and not own friendship", async () => {
      const response = await app.inject({
        method: "DELETE",
        url: "/users/friends/3",
        headers: { "x-user-id": "1" },
      ...
      expect(JSON.parse(response.payload).message).toContain("Forbidden");
    });
  ...
  describe("PUT /users/friends/:targetId", () => {
    test("Should update friend nickname - 200", async () => {
      const updatedFriendship = { ...mockFriendship, nickname: "MyBuddy" };
      vi.spyOn(friendsService, "updateFriend").mockResolvedValue(
      ...
      expect(response.statusCode).toBe(200);
      expect(JSON.parse(response.payload).nickname).toBe("MyBuddy");
    });

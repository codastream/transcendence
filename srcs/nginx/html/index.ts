...
class TranscendenceApp {
  ...
  private showGameOverDialog(gameData: GameState) {
    ...
    if (gameData && gameData.scores) {
      if (scoreP1) scoreP1.textContent = String(gameData.scores.left || 0)
      if (scoreP2) scoreP2.textContent = String(gameData.scores.right || 0)

      // Determine winner
      if (winnerMsg) {
        if (gameData.scores.left > gameData.scores.right) {
          winnerMsg.textContent = '🎉 Player 1 Wins!'
        } else if (gameData.scores.right > gameData.scores.left) {
    ...
    const exitBtn = document.getElementById('close-dialog-btn')
  ...
  private async loadGame(createSocket: boolean): Promise<void> {
    try {
      ...
      if (response.ok && data.sessionId) {
        ...
        if (createSocket) await this.createWebSocketConnection(this.sessionId)
